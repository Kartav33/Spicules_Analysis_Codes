#This code will generate a 2D slice without overlappinng it 
import vtk
...
# Define a callback function for fitting the parabola
def fit_parabola():
    ...
    patch_data = np_array[round(min(y_coords)):round(max(y_coords))+1, round(min(x_coords)):round(max(x_coords))+1, id]
    ...
    ax.text(x_coords[0], y_coords[0] + 0.5, equation, color='b', fontsize=10)

    # Refresh the plot
    fig.canvas.draw()

# Call the function to fit the parabola
fit_parabola()
